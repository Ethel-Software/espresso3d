 ...
 * @author Curt
 *
 * The red plasma particle used in v0.2 logos particle system
 */
public class RedPlasmaParticle extends E3DParticle 
{
    private E3DLight light;

    public RedPlasmaParticle(E3DEngine engine, E3DVector3F startPosition, E3DVector3F startMovementDirection, double velocity, double life, double startDelay, double size, boolean createLight)
    {
        super(engine,startPosition, startMovementDirection,
				velocity, life, 1, 1, size, size, 0, 
				startDelay, "REDPLASMAPARTICLE");        

        if(createLight)
        {
		    light = new E3DLight(engine, ("light" + Math.random()));
		    light.setColor(new E3DVector4F(1.0, 0.0, 0.0, 1.0));
    ...
    public E3DParticle onGetClone() throws Exception {
        return new RedPlasmaParticle(getEngine(), new E3DVector3F(getStartPosition()), new E3DVector3F(getStartMovementDirection()), getVelocity(), getLife(), getStartDelay(), getSize(), getLight() != null);
    }
